...
static const struct ov5647_datafmt ov5647_colour_fmts[] = {
	...
	//{MEDIA_BUS_FMT_UYVY8_2X8, V4L2_COLORSPACE_JPEG},//az force set it
			  ...
			  struct v4l2_subdev_format *format)
{
...
#else
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov5647 *ov5647 = to_ov5647(client);
	struct v4l2_mbus_framefmt *mf;

	dev_dbg(&client->dev, "ov5647_get_fmt\n");
    ...
    mf=&format->format;
    //ov5647_mode_info_data
    mf->width = 800;
    mf->height = 600;
    mf->code = MEDIA_BUS_FMT_SBGGR8_1X8;// ov5647_colour_fmts[0];
    mf->colorspace = V4L2_COLORSPACE_SRGB;//ov5647_colour_fmts[1];//;
    mf->field = V4L2_FIELD_NONE;

	printk("embest_debug: ov5647_get_fmt: %x %dx%d\n",
			...
			const struct i2c_device_id *id)
{
...
#if 1
    printk("az~~~~~~~~~~~~0x%x:\n",gpio_get_value_cansleep(pwn_gpio));
    printk("az~~~~~~~~~~~~0x%x:\n",gpio_get_value_cansleep(rst_gpio));
#endif
	...
	ov5647_data.sensor_clk = devm_clk_get(&client->dev, "vin1a_refclk");
	if (IS_ERR(ov5647_data.sensor_clk)) {
		dev_err(dev, "get vin1a_refclk failed\n");
		return PTR_ERR(ov5647_data.sensor_clk);
    }

	ov5647_data.mclk = clk_get_rate(ov5647_data.sensor_clk);
	if (ov5647_data.mclk < 6000000 ||
	    ov5647_data.mclk > 27000000)
		return -EINVAL;
#else
	...
	//ov5647_stream_off();//az
	//retval = ov5647_write_reg(0x503D, 0x80);//az test color-bar
	//retval = ov5647_write_reg(0x503D, 0x81);//az test color-square
